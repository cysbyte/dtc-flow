...
const OpacityOption: FC<OpacityProps> = (props) => {
  const [percent, setPercent] = useState(props.opacity || 100);
  const [enableDrag, setEnableDrag] = useState(false);
  const [left, setLeft] = useState(props.opacity || 100);
  const [width, setWidth] = useState(props.opacity || 100);
